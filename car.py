...
class Car(object):  ## Object are always CAPITALIZED  so we see [C] in [Car] is capital here.
    
    
    # Car constructor 
    # Called when you write car.Car(_, _, _)					## this init function is responsible for creating some space in memory
    def __init__(self, position, velocity, world,color = 'r'):  ##  to make a specific car object like Ahmed or Hafez.
    ## we add a new input color paramter with default to be red.
        ...
        # Initalize the path
        self.path = []   	## list of car location the car has visited that we can then visualize. 
    ...
    def move(self, dt=1):
        ...
        height = len(self.world)          ### len(self.world) Give us a number of rows
        width = len(self.world[0])		  ### len(self.world[0]) gives us a number of colomns
    ...
    def turn_right(self):
        ...
        predicted_velocity = [     ### Don't Forget that Vy is first element and Vx is second element
            velocity[1],
            -velocity[0]
        ]
